...
## 구조
```
index.html                 홈 (v2 2026-09-17: hero 문장+관리기록 예시 패널 → 제공 서비스 3행(#services) → 일하는 방식(#why) → 확인 필요 농지(#check) → 조사 안내 펼쳐보기(#guide/#facts) → 뉴스 3+더보기 12(#news) → 상담 폼(#apply))
services.html              서비스소개 (v2: page-hero → 목차 subnav → 제공 서비스(#diagnosis/#planning/#farming) → 일하는 방식 → 확인 필요 농지+사진 → 처분 절차·이행강제금·관리 안내 펼쳐보기(#legal-guide/#process/#manage) → 폼)
